...
const fetchUser: Fetcher<User, string> = (url) =>
	fetch(url).then((res) => res.json() as Promise<User>)

const fetchTeam: Fetcher<TeamDataWithMembers, string> = (url) =>
	fetch(url).then((res) => res.json() as Promise<TeamDataWithMembers>)
...
function ManageSubscription() {
	const { data: teamData } = useSWR<TeamDataWithMembers>('/api/team', fetchTeam, { suspense: true })
...
function InviteTeamMember() {
	const { data: user } = useSWR<User>('/api/user', fetchUser, { suspense: true })
...
export default function SettingsPage() {
	return (
		<section className="flex-1 p-4 lg:p-8">
			...
			<Suspense fallback={<SubscriptionSkeleton />}><ManageSubscription /></Suspense>
			<Suspense fallback={<TeamMembersSkeleton />}><TeamMembers /></Suspense>
			<Suspense fallback={<InviteTeamMemberSkeleton />}><InviteTeamMember /></Suspense>
		</section>
	)
}
